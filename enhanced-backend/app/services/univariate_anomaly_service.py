   ...
   ✅ No anomalies detected using this method."""
    ...
    def _generate_advanced_anomaly_charts(self, df: pd.DataFrame, anomaly_results: Dict[str, Any], numeric_columns: List[str], parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Individual column analysis (limit to first 3 columns for performance)
            columns_to_plot = numeric_columns[:3]
            
            for col in columns_to_plot:
                charts[f'{col}_analysis'] = {
                    'chart': self._create_column_analysis_chart(df, anomaly_results, col),
                    'insight': self._generate_column_analysis_insight(df, anomaly_results, col)
                }
            
            # Time series analysis
            charts['time_series_analysis'] = {
                'chart': self._create_time_series_chart(df, anomaly_results, columns_to_plot),
                'insight': self._generate_time_series_insight(df, anomaly_results, columns_to_plot)
            }
            
            # Anomaly distribution analysis
            charts['anomaly_distribution'] = {
                'chart': self._create_anomaly_distribution_chart(anomaly_results, numeric_columns),
                'insight': self._generate_distribution_insight(anomaly_results, numeric_columns)
            }
    ...
    def _create_statistical_overview_chart(self, df: pd.DataFrame, anomaly_results: Dict[str, Any], numeric_columns: List[str]) -> str:
        """Create statistical overview chart"""
        # Calculate statistical metrics for each column
        stats_data = []
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=['Mean vs Std Deviation', 'Skewness Distribution', 
                          'Anomaly Rate by Column', 'Anomaly Count vs Data Quality'],
            specs=[[{'type': 'scatter'}, {'type': 'bar'}],
                   [{'type': 'bar'}, {'type': 'scatter'}]]
        )
        
        # Chart 1: Mean vs Std scatter
        fig.add_trace(
            go.Scatter(
                x=[d['mean'] for d in stats_data],
                y=[d['std'] for d in stats_data],
                mode='markers+text',
                text=[d['column'] for d in stats_data],
                textposition="top center",
        ...
        # Chart 2: Skewness distribution
        fig.add_trace(
            go.Bar(
                x=[d['column'] for d in stats_data],
                y=[d['skewness'] for d in stats_data],
                name='Skewness',
                marker_color='orange'
            ),
            row=1, col=2
        ...
        # Chart 3: Anomaly rate by column
        fig.add_trace(
            go.Bar(
                x=[d['column'] for d in stats_data],
        ...
        # Chart 4: Anomaly count vs data quality (inverse of std)
        fig.add_trace(
            go.Scatter(
                x=[1/d['std'] if d['std'] > 0 else 0 for d in stats_data],
                y=[d['total_anomalies'] for d in stats_data],
                mode='markers+text',
                text=[d['column'] for d in stats_data],
                textposition="top center",
                marker=dict(size=10, color='green'),
                name='Quality vs Anomalies'
            ),
            row=2, col=2
        )
        ...
        fig.update_layout(
            height=800,
            title_text="Statistical Overview and Data Quality Analysis",
            title_x=0.5,
            showlegend=False
        )
...
Use distribution width to understand method reliability. Consistent methods (narrow distributions) are good for standardized monitoring, while variable methods (wide distributions) might be better for detecting column-specific anomaly patterns.
    ...
    def _generate_statistical_overview_insight(self, df: pd.DataFrame, anomaly_results: Dict[str, Any], numeric_columns: List[str]) -> str:
        """Generate insight for statistical overview"""
        try:
            # Calculate correlations between data characteristics and anomaly rates
            correlations = {}
            ...
            # Find patterns
            high_anomaly_columns = [col for col, data in correlations.items() if data['anomaly_rate'] > 5]
            highly_skewed_columns = [col for col, data in correlations.items() if abs(data['stats']['skewness']) > 1]
            
            return f"""
📊 **Statistical Overview & Data Quality Assessment**

**Quality Metrics:**
• {len(high_anomaly_columns)} columns with high anomaly rates (>5%)
• {len(highly_skewed_columns)} columns with significant distribution skewness
• {len(numeric_columns)} total columns analyzed for statistical relationships

**Statistical Relationship Analysis:**
The four-panel view shows: (1) Mean vs Standard Deviation scatter, (2) Skewness distribution, (3) Anomaly rates by column, and (4) Quality vs Anomaly relationship. These reveal how statistical properties correlate with anomaly detection.

**Key Patterns:**
• Higher standard deviation often correlates with increased anomaly detection
• Skewed distributions (values far from 0) tend to produce more outliers in tail regions
• Data quality (inverse of variability) generally correlates with lower anomaly rates
• Outliers in the scatter plots indicate columns with unusual statistical properties

**Data Health Indicators:**
{"Several columns show concerning anomaly patterns - focus on: " + ", ".join(high_anomaly_columns[:3]) if high_anomaly_columns else "Anomaly rates are within normal ranges across all columns"}
{"Distribution skewness detected in multiple columns - consider normalization" if highly_skewed_columns else "Data distributions appear reasonably symmetric"}
            """.strip()
        except Exception as e:
    ...
    def _generate_box_plots_insight(self, df: pd.DataFrame, anomaly_results: Dict[str, Any], columns: List[str]) -> str:
        ...
        try:
            ...
            return f"""
📦 **Box Plot Analysis with Anomaly Detection**
